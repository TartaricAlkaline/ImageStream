...
final class LoopFilter {
   ...
   static void vp8_loop_filter_frame_init(CommonData cm, MacroblockD mbd, short default_filt_lvl) {
      ...
      for (int seg = 0; seg < 4; seg++) {
         ...
         if (mbd.segmentation_enabled != 0) {
            if (mbd.mb_segement_abs_delta) {
               lvl_seg = mbd.segment_feature_data[MBLvlFeatures.ALT_LF.ordinal()][seg];
            } else {
               lvl_seg = (short)(default_filt_lvl + mbd.segment_feature_data[MBLvlFeatures.ALT_LF.ordinal()][seg]);
            }
         ...
         if (!mbd.mode_ref_lf_delta_enabled) {
            ...
            int mode = 0;
            short lvl_mode = (short)(lvl_ref + mbd.mode_lf_deltas[mode]);
            lvl_mode = lvl_mode > 0 ? (lvl_mode > 63 ? 63 : lvl_mode) : 0;
            lfi.lvl[seg][ref][mode] = lvl_mode;
            int var12 = 1;
            lvl_mode = (short)(lvl_ref > 0 ? Math.min(lvl_ref, 63) : 0);
            lfi.lvl[seg][ref][var12] = lvl_mode;

            for (int var11 = 1; var11 < MVReferenceFrame.count; var11++) {
               lvl_ref = lvl_seg + mbd.ref_lf_deltas[var11];

               for (int var13 = 1; var13 < 4; var13++) {
                  lvl_mode = (short)(lvl_ref + mbd.mode_lf_deltas[var13]);
                  lvl_mode = lvl_mode > 0 ? (lvl_mode > 63 ? 63 : lvl_mode) : 0;
                  lfi.lvl[seg][var11][var13] = lvl_mode;
               }
            }
         }
      }
   ...
   static void filterThrough(
      LoopFilterCore actualFilter,
      PtrIncrementor incrementor,
      CommonData cm,
      int frame_type,
      int mb_rows,
      int mb_cols,
      FullGetSetPointer y_ptr,
      FullGetSetPointer u_ptr,
      FullGetSetPointer v_ptr,
      FullGenArrPointer<ModeInfo> mode_info_context,
      boolean dontSkipFirstRow
   ) {
      ...
      for (int mb_row = 0; mb_row < mb_rows; mb_row++) {
         for (int mb_col = 0; mb_col < mb_cols; mb_col++) {
            ...
            int mode_index = lfi_n.mode_lf_lut.get(mi.mbmi.mode);
            int seg = mi.mbmi.segment_id;
            MVReferenceFrame ref_frame = mi.mbmi.ref_frame;
            int filter_level = lfi_n.lvl[seg][ref_frame.ordinal()][mode_index];
            if (filter_level != 0) {
   ...
   static void vp8_loop_filter_frame(CommonData cm, MacroblockD mbd, int frame_type) {
      ...
      filterThrough(
         cm.filter_type == 0 ? normalCore : simpleCore,
         regularIncrementor,
         cm,
         frame_type,
         cm.mb_rows,
   ...
   static void vp8_loop_filter_frame_yonly(CommonData cm, MacroblockD mbd, short default_filt_lvl) {
      YV12buffer post = cm.frame_to_show;
      vp8_loop_filter_partial_frame_spec(cm, mbd, default_filt_lvl, cm.mb_rows << 4, post.y_buffer.shallowCopy(), cm.mi.shallowCopy(), false);
   }

   private static void vp8_loop_filter_partial_frame_spec(
      CommonData cm, MacroblockD mbd, short default_filt_lvl, int linestocopy, FullGetSetPointer y_ptr, FullGenArrPointer<ModeInfo> mic, boolean alwaysMBH
   ) {
      vp8_loop_filter_frame_init(cm, mbd, default_filt_lvl);
      filterThrough(
         cm.filter_type == 0 ? normalCore : yOnlySimpleCore,
         yOnlyIncrementor,
         cm,
         cm.frame_type,
         linestocopy >> 4,
   ...
   interface LoopFilterCore {
      void call(
         int var1,
         int var2,
         LoopFilterInfoN var3,
         boolean var4,
         int var5,
         int var6,
         FullGetSetPointer var7,
         int var8,
         FullGetSetPointer var9,
         FullGetSetPointer var10,
         int var11,
         boolean var12
      );
   }
   ...
   interface PtrIncrementor {
      void nextMB(FullGetSetPointer var1, FullGetSetPointer var2, FullGetSetPointer var3);

      void nextRow(FullGetSetPointer var1, FullGetSetPointer var2, FullGetSetPointer var3, YV12buffer var4);
   }
}
